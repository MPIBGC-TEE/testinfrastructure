...
# but is a known problem at the time of writing and does affect us.
 micromamba install -y -c conda-forge nbconvert jupytext # this requirement file is conda specific # it only lists those packages that are not extractable from setup.py # which are extracted by the installer automatically pip
...
# you need some extra packages which can be installed by uncommenting the following line:
 micromamba install -y -c conda-forge matplotlib
...
# The following line installs the package without checking out the repository directly from github
 pip install --no-deps git+https://github.com/MPIBGC-TEE/testinfrastructure.git#egg=testinfrastructure
