...
# but is a known problem at the time of writing and does affect us.
 mamba install -y -c conda-forge nbconvert jupytext # this requirement file is conda specific # it only lists those packages that are not extractable from setup.py # which are extracted by the installer automatically pip
